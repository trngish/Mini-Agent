...
class ThinkingManager:
    ...
    def analyze_messages(self, messages: list[Message]) -> dict[str, Any]:
        ...
        if total_tokens > self.prune_threshold:
            ...
            candidates = [
                i for i in thinking_by_msg.keys()
                if i > 0 and thinking_by_msg[i] > 0 and i < len(messages) - 2
            ]
            candidates.sort()
    ...
    def _create_thinking_summary(self, thinking: str, encoder) -> str:
    ...
    def _truncate_thinking(self, thinking: str, encoder) -> str:
        ...
        return truncated + f"\n... [truncated from {len(tokens)} tokens]"
