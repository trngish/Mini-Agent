...
class DocxXMLEditor(XMLEditor):
    """XMLEditor，自动向新元素应用 RSID、作者和日期。

    插入新内容时，自动向支持这些属性的元素添加属性：
    ...
    def _inject_attributes_to_nodes(self, nodes):
        ...
        向支持这些属性的元素添加属性：
        - w:r：获取 w:rsidR（或如果在 w:del 内则获取 w:rsidDel）
        ...
        def add_rsid_to_r(elem):
            # 对于在 <w:del> 内的 <w:r> 使用 w:rsidDel，否则使用 w:rsidR
        ...
        def add_tracked_change_attrs(elem):
            ...
            # 为修订跟踪添加 w16du:dateUtc（与 w:date 相同，因为我们生成 UTC 时间戳）
        ...
        def add_comment_extensible_date(elem):
            # 为 comment extensible 元素添加 w16cex:dateUtc
        ...
        def add_xml_space_to_t(elem):
            # 如果文本有前导/尾随空格则向 w:t 添加 xml:space="preserve"
        ...
        for node in nodes:
            ...
            # 处理后代（getElementsByTagName 不返回元素本身）
        ...
        for ins_elem in ins_elements:
            ...
            for run in runs:
                ...
                for t_elem in list(run.getElementsByTagName("w:t")):
                    ...
                    # 复制所有子节点（而不仅仅是 firstChild）以处理实体
    ...
    def revert_deletion(self, elem):
        ...
        在每个 w:del 后创建 w:ins 元素，复制已删除的内容并将 w:delText 转换回 w:t。
    ...
    def suggest_paragraph(xml_content: str) -> str:
        """转换段落 XML 以添加插入的修订跟踪包装。

        将运行包装在 <w:ins> 中，并在编号列表的 w:pPr 的 w:rPr 中添加 <w:ins/>。

        参数：
            xml_content：包含 <w:p> 元素的 XML 字符串

        返回：
            str：带有修订跟踪包装的转换后 XML
        """
    ...
    def suggest_deletion(self, elem):
        """将 w:r 或 w:p 元素标记为已删除（原地 DOM 操作）。

        对于 w:r：包装在 <w:del> 中，转换 <w:t> 为 <w:delText>，保留 w:rPr
        对于 w:p（普通）：将内容包装在 <w:del> 中，转换 <w:t> 为 <w:delText>
        对于 w:p（编号列表）：向 w:pPr 中的 w:rPr 添加 <w:del/>，将内容包装在 <w:del> 中
        ...
        if elem.nodeName == "w:r":
            ...
            raise ValueError(f"元素必须是 w:r 或 w:p，实际为 {elem.nodeName}")


def _generate_hex_id() -> str:
    """为段落/持久 ID 生成随机的 8 位十六进制 ID。

    值被限制为小于 0x7FFFFFFF（根据 OOXML 规范）：
    - paraId 必须 < 0x80000000
    - durableId 必须 < 0x7FFFFFFF
    我们对两者使用更严格的约束（0x7FFFFFFF）。
    """
...
class Document:
    ...
    def __init__(
        ...
        """
        使用未打包的 Word 文档目录路径进行初始化。
        自动设置批注基础设施（people.xml、RSIDs）。

        参数：
            unpacked_dir：未打包的 DOCX 目录路径（必须包含 word/ 子目录）
            rsid：可选的 RSID，用于所有批注元素。如果未提供，将生成一个。
            track_revisions：如果为 True，则在 settings.xml 中启用修订跟踪（默认：False）
            author：批注的默认作者名（默认："Claude"）
        ...
        # 将原始目录打包为临时的 .docx 用于验证基准（在 unpacked dir 之外）
    ...
    def __getitem__(self, xml_path: str) -> DocxXMLEditor:
        ...
        if xml_path not in self._editors:
            ...
            # 对所有编辑器使用带有 RSID、作者和缩写的 DocxXMLEditor
    ...
    def reply_to_comment(
        ...
        示例：
            cm.reply_to_comment(parent_comment_id=0, text="我同意此更改")
        """
    ...
    def _setup_tracking(self, track_revisions=False):
        ...
        参数：
            track_revisions：如果为 True，则在 settings.xml 中启用 trackRevisions
        """
        ...
        # 始终将 RSID 添加到 settings.xml，可选择启用 trackRevisions
    ...
    def _add_content_type_for_people(self, path):
        """如果 people.xml 内容类型尚未存在则将其添加到 [Content_Types].xml。"""
    ...
    def _add_relationship_for_people(self, path):
        """如果 people.xml 关系尚未存在则将其添加到 document.xml.rels。"""
    ...
    def _update_settings(self, path, track_revisions=False):
        """在 settings.xml 中添加 RSID 并可选启用修订跟踪。

        参数：
            path：settings.xml 的路径
            track_revisions：如果为 True，则添加 trackRevisions 元素
  ...
  <{prefix}:rsid {prefix}:val="{self.rsid}"/>
            ...
            # 尝试在 compat 之后、clrSchemeMapping 之前或结束标签之前插入
            inserted = False
...
<w:r>
  ...
  <w:commentReference w:id="{comment_id}"/>
    ...
    def _add_author_to_people(self, author):
        ...
        # people.xml 应该已从 _setup_tracking 存在
        if not people_path.exists():
            raise ValueError("_setup_tracking 之后 people.xml 应该存在")
