#!/usr/bin/env python3
"""
将目录打包为 .docx、.pptx 或 .xlsx 文件的工具，并撤销 XML 格式设置。
...
def main():
    ...
    try:
        ...
        # 如果验证失败则退出并出错
            ...
            print("请在重新打包之前验证 XML。", file=sys.stderr)
...
def pack_document(input_dir, output_file, validate=False):
    ...
    参数：
        input_dir：未打包的 Office 文档目录路径
        output_file：输出 Office 文件路径
        validate：如果为 True，则用 soffice 验证（默认：False）

    返回：
        bool：如果成功则返回 True，验证失败则返回 False
    """
