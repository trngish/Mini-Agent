...
*   ✅ **语义跨会话记忆**：自动从对话中提取决策、偏好和发现，跨会话持久化并在新会话中注入上下文。
*   ✅ **智能上下文管理**：带反退化保护的自动摘要，确保 AI 不会"忘记"自己刚说过的内容；支持可配置 Token 上限。
*   ✅ **自愈引擎**：运行时检测自身异常（循环检测、LLM 错误、健康问题），可选自动诊断并修复源代码（`MINI_AGENT_AUTO_HEAL=1`）。
*   ✅ **工作区隔离**：缓存和会话按工作区命名空间隔离，多项目并行运行不互相污染。
*   ✅ **双协议流式输出**：Anthropic 和 OpenAI 协议均支持流式输出，思考内容先于结果输出，保证阅读体验。
*   ✅ **完整会话恢复**：会话保存时持久化完整运行时状态（Token 计数、思考预算、循环计数），恢复后无需"重新热身"。
*   ✅ **集成 Claude Skills**：内置 15 种专业技能，涵盖文档处理、设计、测试和开发等领域。
*   ✅ **集成 MCP 工具**：原生支持 MCP 协议，可轻松接入知识图谱、网页搜索等工具。
*   ✅ **全面的日志记录**：为每个请求、响应和工具执行提供详细日志，健康检查、循环检测等关键事件可审计。
...
Mini-Agent 在执行过程中持续监控自身状态，检测异常模式（循环检测、LLM 错误、健康问题、Token 压力等），当异常分数超过阈值时自动触发诊断。
...
修复的源码变更在下次启动时生效（因为 `uv tool install -e .` 编辑式安装）。
所有修复都有备份（`~/.mini-agent/heal_backups/`），支持回滚。

### 环境变量

| 变量 | 默认值 | 说明 |
|------|--------|------|
| `MINI_AGENT_AUTO_HEAL` | `0` | 启用自愈引擎自动修复（1=开启） |
| `MINI_AGENT_AUTO_SAVE` | `true` | 自动保存会话 |
| `MINI_AGENT_SESSION_ISOLATION` | `1` | 按工作区隔离会话存储 |
| `MINI_AGENT_MAX_SESSIONS` | `100` | 最大保留会话数（超限自动清理） |
| `MINI_AGENT_STREAM_BUFFER_SIZE` | `8` | 流式输出缓冲区大小 |
| `MINI_AGENT_CACHE_WARMUP` | `true` | 启动时预热文件缓存 |
